...
const Player = ({
   ...
   useEffect(() => {
      ...
      const eventWrapper = (type, val, fcn) => {
         return (e) => {
            const ret = fcn(e)
      ...
      const setAudioData = eventWrapper("load", {audio}, () => {
         setLength(audio.duration)
         setTime(audio.currentTime)
      })

      const setAudioTime = eventWrapper("time", {audio}, () => {
         const curTime = audio.currentTime
         setTime(curTime)
         setSlider(curTime ? ((curTime * 100) / audio.duration).toFixed(1) : 0)
      })

      const setAudioProgress = eventWrapper("progress", {audio}, () => {
         const bufferedPercentage = (audio.buffered.end(0) / audio.duration) * 100
         setBuffer(bufferedPercentage.toFixed(2))
      })

      const setAudioVolume = eventWrapper("volume", {audio}, () => setVolume(audio.volume))
      const setAudioEnd = eventWrapper("ended", {audio}, () => setHasEnded(!hasEnded))
   ...
   const sortCompare = (a, b) => (!sortTracks ? null : a.title.toLowerCase() > b.title.toLowerCase() ? 1 : -1)
   ...
   return (
      <PageTemplate>
         ...
         <PlayerTemplate>
            <TitleAndTimeBox>
               <Title title={title} />
               <Time time={`${!time ? "0:00" : fmtMSS(time)}/${!length ? "0:00" : fmtMSS(length)}`} />
            </TitleAndTimeBox>
            <Progress
               value={slider}
               progress={buffer}
            ...
            <ButtonsAndVolumeBox>
               <ButtonsBox>
                  ...
                  {isPlaying ? <Pause src={pauseBtn} onClick={pause} /> : <Play src={playBtn} onClick={play} />}
